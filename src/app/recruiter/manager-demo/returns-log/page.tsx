// src/app/recruiter/manager-demo/returns-log/page.tsx
"use client";
...
export default function ManagerReturnsLogPage() {
  ...
  const handleManagerMarkAsReturned = async (transaction: SaleToReturn) => {
    if (!transaction) { 
        sonnerToast.error("No transaction selected."); 
        return; 
    }
    if (transaction.status !== "SOLD") { 
        sonnerToast.warning(`Item is already ${transaction.status}. Cannot mark as returned again.`); 
        return; 
    }
    ...
    setTimeout(() => {
        ...
        // Reset loading state and close the modal after the toast appears
        setIsUpdatingStatus(null);
        setIsMarkReturnModalOpen(false); 
        // Note: The call to fetchReturnsLog() is removed as no data actually changes.
