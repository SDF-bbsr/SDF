// src/app/recruiter/vendor-demo/sales-history/page.tsx
"use client";
...
import { Loader2, ArrowLeft, List, CalendarDays, RefreshCw, ChevronLeft, ChevronRight, Target, TrendingUp, LogOut } from 'lucide-react'; // Added LogOut for Exit button
...
const demoUser = {
    id: 'Parimita', // A static ID your API can recognize, or a real one with data
    name: 'Parimita'
};

export default function VendorSalesHistoryDemoPage() {
  // REMOVED: const { user, logout } = useUser();
  const router = useRouter();

  const [overallStats, setOverallStats] = useState<OverallStats | null>(null);
  const [currentWeekTarget, setCurrentWeekTarget] = useState<CurrentWeekTargetInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchOverallData = async () => {
      ...
      try {
        // MODIFIED: Uses demoUser object instead of context
        const response = await fetch(`/api/sales/vendor-history?staffId=${demoUser.id}&staffName=${encodeURIComponent(demoUser.name)}&mode=stats`);
        if (!response.ok) {
        ...
        setCurrentWeekTarget(data.currentWeekTarget);
      } catch (err: any) { setError(err.message); toast.error(err.message); }
      finally { setIsLoadingStats(false); }
    };
    fetchOverallData();
  }, []); // REMOVED: user and router dependencies, as they are static now.

  const fetchDailySummaries = useCallback(async (start: string, end: string) => {
    // REMOVED: user check
    if (!start || !end) return;
    setIsLoadingDaily(true); setError(null);
    // MODIFIED: Uses demoUser object
    const params = new URLSearchParams({ staffId: demoUser.id, mode: 'dailySummaries', startDate: start, endDate: end });
    try {
  ...
  const fetchTransactions = useCallback(async (periodType: TransactionPeriod, page: number = 1, customSDate?: string, customEDate?: string) => {
    ...
    // MODIFIED: Uses demoUser object
    const params = new URLSearchParams({ staffId: demoUser.id, mode: 'transactions', page: String(page), limit: '30' });
    let sDate = '', eDate = '';
    const nowIST = getNowInClientIST();
    ...
    params.append('startDate', sDate); params.append('endDate', eDate);
    try {
      const response = await fetch(`/api/sales/vendor-history?${params.toString()}`);
  ...
  // handleCustomDateFilterApply and other component logic remains the same
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 bg-slate-50 dark:bg-slate-900">
        <div className="max-w-6xl mx-auto">
            ...
            {/* MODIFIED: Link points back to the demo scan page */}
            <Link href="/recruiter/vendor-demo/scan" passHref>
                <Button variant="outline" size="sm"><ArrowLeft className="mr-2 h-4 w-4" /> Back to POS Demo</Button>
            </Link>
            {/* MODIFIED: Uses static demoUser name */}
            <h1 className="text-xl md:text-2xl font-semibold">Sales Dashboard - {demoUser.name}</h1>
            {/* MODIFIED: "Logout" is now "Exit Demo" and goes to recruiter portal */}
            <Button variant="ghost" size="sm" onClick={() => router.push('/recruiter/portal')}>
                <LogOut className="mr-2 h-4 w-4" /> Exit Demo
            </Button>
            </div>

            {/* Rest of the JSX remains the same, as it's read-only by nature */}
            {/* Stats Cards */}
