...
import { Loader2, Filter, PackageCheck, PackageX, UploadCloud, List, CalendarDays, Download, Copy, FileText, FileSpreadsheet, Info } from 'lucide-react';
...
export default function ManagerSalesLogPage() {
  ...
  const [activeExportTab, setActiveExportTab] = useState<'excel' | 'barcodes'>('excel'); // New state for active export tab
  const [exportFilters, setExportFilters] = useState({
    ...defaultDates, // Default date range for export filters as well
    staffId: 'all',
    sortOrder: 'desc',
  ...
  const fetchStaffList = useCallback(async () => { /* ... (no change) ... */ 
    try {
  ...
  const fetchTransactions = useCallback(async (limit = 1000) => { /* ... (no change) ... */
    ...
    try {
      ...
      if (!response.ok) { const errData = await response.json(); throw new Error(errData.message || errData.details || 'Failed to fetch transactions'); }
      const data = await response.json();
      setTransactions(data.transactions || []);
    } catch (err: any) { setError(err.message); setTransactions([]); sonnerToast.error("Error fetching sales: " + err.message);
    } finally { setIsLoading(false); }
  }, [appliedFilters]);

  useEffect(() => { fetchTransactions(); fetchStaffList(); }, [fetchTransactions, fetchStaffList]);
  useEffect(() => { /* ... (daily summary calculation - no change) ... */
    if (transactions.length > 0) {
      const summaries: { [date: string]: Omit<DailySaleSummary, 'date' | 'avgPacketValue'> & { items: SaleTransaction[] } } = {};
      transactions.forEach(tx => { const date = tx.dateOfSale; if (!summaries[date]) { summaries[date] = { totalSaleValue: 0, totalPacketsSold: 0, items: [] }; } summaries[date].totalSaleValue += tx.calculatedSellPrice; summaries[date].totalPacketsSold += 1; summaries[date].items.push(tx); });
      const formattedSummaries: DailySaleSummary[] = Object.entries(summaries).map(([date, data]) => ({ date, totalSaleValue: data.totalSaleValue, totalPacketsSold: data.totalPacketsSold, avgPacketValue: data.totalPacketsSold > 0 ? data.totalSaleValue / data.totalPacketsSold : 0, })).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const parseBarcode = (barcode: string): { articleNo: string; weightGrams: number } | null => { /* ... (no change) ... */ 
    barcode = barcode.trim(); if (barcode.length < BARCODE_PREFIX_LENGTH + ARTICLE_NO_LENGTH + WEIGHT_GRAMS_LENGTH) return null;
    try { const articleNo = barcode.substring(BARCODE_PREFIX_LENGTH, BARCODE_PREFIX_LENGTH + ARTICLE_NO_LENGTH); const weightStr = barcode.substring(BARCODE_PREFIX_LENGTH + ARTICLE_NO_LENGTH, BARCODE_PREFIX_LENGTH + ARTICLE_NO_LENGTH + WEIGHT_GRAMS_LENGTH); const weightGrams = parseInt(weightStr, 10); if (isNaN(weightGrams) || !/^\d+$/.test(articleNo)) return null; return { articleNo, weightGrams };
    } catch (e) { return null; }
  };
  const handleBulkSalesSubmit = async () => { /* ... (no change) ... */ 
    if (!bulkSalesData.barcodes.trim() || !bulkSalesData.staffId || !bulkSalesData.dateOfSale) { sonnerToast.error("All fields required for bulk sales."); return; }
    const barcodeLines = bulkSalesData.barcodes.trim().split('\n'); const salesToRecord: any[] = []; let parseErrors = 0;
    for (const line of barcodeLines) { const barcode = line.trim(); if (!barcode) continue; const parsed = parseBarcode(barcode); if (parsed) salesToRecord.push({ ...parsed, barcodeScanned: barcode, staffId: bulkSalesData.staffId, dateOfSale: bulkSalesData.dateOfSale }); else { parseErrors++; sonnerToast.warning(`Invalid barcode: "${barcode}"`); }}
    if (parseErrors > 0 && !confirm(`${parseErrors} invalid barcodes. Proceed?`)) return;
    if (salesToRecord.length === 0) { sonnerToast.info("No valid sales."); return; }
    setIsSubmittingBulk(true);
    try { const response = await fetch('/api/manager/sales-transactions/bulk-record', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sales: salesToRecord }), }); const result = await response.json(); if (!response.ok) throw new Error(result.message || "Bulk recording failed."); sonnerToast.success(`${result.successfulRecords || 0} sales recorded. ${result.failedRecords || 0} failed.`); setIsBulkAddModalOpen(false); setBulkSalesData(prev => ({ ...prev, barcodes: '' })); fetchTransactions();
    } catch (err: any) { sonnerToast.error("Bulk sales error: " + err.message); } finally { setIsSubmittingBulk(false); }
  };
  const getStatusBadge = (status: string) => { /* ... (no change) ... */ 
    switch (status) { case "SOLD": return <span className="px-2 py-0.5 text-xs font-medium text-green-800 bg-green-100 rounded-full inline-flex items-center gap-1"><PackageCheck className="h-3 w-3" /> {status}</span>; case "RETURNED_PRE_BILLING": return <span className="px-2 py-0.5 text-xs font-medium text-red-800 bg-red-100 rounded-full inline-flex items-center gap-1"><PackageX className="h-3 w-3" /> RETURNED</span>; default: return <span className="px-2 py-0.5 text-xs font-medium text-gray-800 bg-gray-100 rounded-full">{status}</span>; }
  };
  ...
  const fetchExportData = async (forPreviewCountOnly = false, forBarcodeTextExport = false): Promise<SaleTransaction[] | number> => { /* ... (no change) ... */
    setIsExporting(true); if (!forPreviewCountOnly) setExportedBarcodesText(''); // Clear only when fetching full data for barcodes
    if (!forPreviewCountOnly && forBarcodeTextExport) { // Clear only when fetching full data for barcodes text specifically
    ...
    queryParams.append('limit', forPreviewCountOnly ? '0' : '10000'); // 0 for countOnly in API
    ...
    try {
      ...
      if (!response.ok) { const errData = await response.json(); throw new Error(errData.message || "Failed to fetch data for export"); }
      const data = await response.json();
      if (forPreviewCountOnly) {
        const count = data.totalRecords === -1 ? 1 : data.totalRecords || 0; // Handle -1 from API for "some exist"
        ...
        if (forBarcodeTextExport) {
            const barcodes = fetchedTransactions
                .map(tx => String(tx.barcodeScanned || '')) // Ensure it's treated as a string, handle null/undefined
                .filter(barcode => barcode.trim() !== "") // Filter out empty strings after conversion
                .join('\n');
  ...
  const handleGenerateExcel = async () => {
    setIsExporting(true); // Ensure loading state
    const dataToExport = await fetchExportData(false, false) as SaleTransaction[];
    setIsExporting(false); // Reset loading state
    if (!dataToExport || dataToExport.length === 0) { sonnerToast.info("No data to export for Excel based on current filters."); return; }
    const selectedHeaders = exportFilters.selectedFields.map(key => ALL_EXPORTABLE_FIELDS.find(f => f.key === key)?.label || String(key));
    const worksheetData = dataToExport.map(tx => { const row: any = {}; exportFilters.selectedFields.forEach(key => { const fieldConfig = ALL_EXPORTABLE_FIELDS.find(f => f.key === key); let value = tx[key] as any; if (key === 'timestamp' && value) value = new Date(value).toLocaleString(); if (key === 'calculatedSellPrice' && typeof value === 'number') value = value.toFixed(2); row[fieldConfig?.label || String(key)] = value !== undefined && value !== null ? value : ''; }); return row; });
    const worksheet = XLSX.utils.json_to_sheet(worksheetData, { header: selectedHeaders });
    const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, worksheet, "SalesData");
    XLSX.writeFile(workbook, `SalesExport_${new Date().toISOString().split('T')[0]}.xlsx`);
    sonnerToast.success("Excel file generation initiated!");
  };

  const handleExportBarcodesAsText = async () => {
    setIsExporting(true); // Ensure loading state
    await fetchExportData(false, true); // This will populate exportedBarcodesText
    setIsExporting(false); // Reset loading state
    // Toast message is now handled within fetchExportData or by checking exportedBarcodesText state
    if (exportedBarcodesText) { sonnerToast.success("Barcodes prepared. See textbox below."); }
    else if(exportDataPreviewCount === 0) { sonnerToast.info("No barcodes to export for the selected criteria."); }
    // If exportDataPreviewCount is > 0 but exportedBarcodesText is empty, it means transactions exist but none have barcodes.
  };
    ...
    <>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div className="flex gap-2">
            <Button onClick={() => setIsBulkAddModalOpen(true)} variant="outline"><UploadCloud className="mr-2 h-4 w-4" /> Bulk Add Sales</Button>
            <Button onClick={() => {
                const currentDefDates = getDefaultDateRange(); // Get fresh defaults when opening
                setExportFilters(prev => ({...prev, startDate: currentDefDates.startDate, endDate: currentDefDates.endDate }));
                setExportDataPreviewCount(null); // Reset preview
                setExportedBarcodesText(''); // Reset text
                setActiveExportTab('excel'); // Default to excel tab
                setIsExportModalOpen(true);
            }} variant="outline"><Download className="mr-2 h-4 w-4" /> Export Data</Button>
        </div>
        <Button onClick={() => setViewMode(prev => prev === 'individual' ? 'daily' : 'individual')} variant="outline">{viewMode === 'individual' ? <CalendarDays className="mr-2 h-4 w-4" /> : <List className="mr-2 h-4 w-4" />}{viewMode === 'individual' ? 'View Daily Summary' : 'View Individual Transactions'}</Button>
      </div>

      <Card className="mb-6">
        <CardHeader><CardTitle className="flex items-center gap-2"><Filter className="h-5 w-5" /> Filter Sales</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          <div><Label htmlFor="startDate">Start Date</Label><Input type="date" id="startDate" value={filters.startDate} onChange={(e) => handleFilterChange('startDate', e.target.value)} /></div>
          <div><Label htmlFor="endDate">End Date</Label><Input type="date" id="endDate" value={filters.endDate} onChange={(e) => handleFilterChange('endDate', e.target.value)} /></div>
          <div><Label htmlFor="staffIdFilter">Staff</Label><Select value={filters.staffId || "all"} onValueChange={(value) => handleFilterChange('staffId', value === "all" ? "" : value)}><SelectTrigger id="staffIdFilter"><SelectValue placeholder="All Staff" /></SelectTrigger><SelectContent><SelectItem value="all">All Staff</SelectItem>{staffList.map(staff => (<SelectItem key={staff.id} value={staff.id}>{staff.name}</SelectItem>))}</SelectContent></Select></div>
          <div><Label htmlFor="statusFilter">Status</Label><Select value={filters.status || "all"} onValueChange={(value) => handleFilterChange('status', value === "all" ? "" : value)}><SelectTrigger id="statusFilter"><SelectValue placeholder="All Statuses" /></SelectTrigger><SelectContent><SelectItem value="all">All Statuses</SelectItem><SelectItem value="SOLD">Sold</SelectItem><SelectItem value="RETURNED_PRE_BILLING">Returned Pre-Billing</SelectItem></SelectContent></Select></div>
          <Button onClick={applyFilters} disabled={isLoading} className="lg:col-start-5 self-end h-10">{isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Filter className="mr-2 h-4 w-4" />}Apply Filters</Button>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          {!isLoading && !error && ( /* ... (table rendering logic - no change) ... */
            viewMode === 'individual' ? ( transactions.length > 0 ? (<ScrollArea className="w-full whitespace-nowrap rounded-md border"><Table className="text-xs"><TableHeader><TableRow><TableHead className="w-[170px] py-2 px-3">Timestamp</TableHead><TableHead className="py-2 px-3">Staff</TableHead><TableHead className="py-2 px-3">Article No</TableHead><TableHead className="py-2 px-3 hidden md:table-cell">Product Name</TableHead><TableHead className="text-right py-2 px-3">Weight (g)</TableHead><TableHead className="text-right py-2 px-3">Price (₹)</TableHead><TableHead className="py-2 px-3">Status</TableHead><TableHead className="hidden lg:table-cell py-2 px-3">Barcode</TableHead></TableRow></TableHeader><TableBody>{transactions.map((tx) => (<TableRow key={tx.id} className="hover:bg-muted/50"><TableCell>{new Date(tx.timestamp).toLocaleString()}</TableCell><TableCell>{tx.staffId}</TableCell><TableCell>{tx.articleNo}</TableCell><TableCell className="hidden md:table-cell truncate max-w-[200px]">{tx.product_articleName || 'N/A'}</TableCell><TableCell className="text-right">{tx.weightGrams}</TableCell><TableCell className="text-right">{tx.calculatedSellPrice.toFixed(2)}</TableCell><TableCell>{getStatusBadge(tx.status)}</TableCell><TableCell className="hidden lg:table-cell truncate max-w-[150px]">{tx.barcodeScanned || 'N/A'}</TableCell></TableRow>))}</TableBody></Table><ScrollBar orientation="horizontal" /></ScrollArea>) : (<p className="text-center py-10 text-muted-foreground">No individual transactions found.</p>) ) : ( dailySummaries.length > 0 ? (<ScrollArea className="w-full whitespace-nowrap rounded-md border"><Table className="text-sm"><TableHeader><TableRow><TableHead className="py-2 px-3">Date</TableHead><TableHead className="text-right py-2 px-3">Packets Sold</TableHead><TableHead className="text-right py-2 px-3">Sale Value (₹)</TableHead><TableHead className="text-right py-2 px-3">Avg. Value (₹)</TableHead></TableRow></TableHeader><TableBody>{dailySummaries.map((summary) => (<TableRow key={summary.date} className="hover:bg-muted/50"><TableCell className="font-medium">{new Date(summary.date + 'T00:00:00').toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</TableCell><TableCell className="text-right">{summary.totalPacketsSold}</TableCell><TableCell className="text-right">₹{summary.totalSaleValue.toFixed(2)}</TableCell><TableCell className="text-right">₹{summary.avgPacketValue.toFixed(2)}</TableCell></TableRow>))}</TableBody></Table><ScrollBar orientation="horizontal" /></ScrollArea>) : (<p className="text-center py-10 text-muted-foreground">No data for daily summary.</p>) )
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={isExportModalOpen} onOpenChange={(isOpen) => {
          setIsExportModalOpen(isOpen);
          if (!isOpen) { setExportDataPreviewCount(null); setExportedBarcodesText(''); setActiveExportTab('excel'); }
      }}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle>Export Sales Data</DialogTitle><DialogDesc>Configure filters and select format for your data export.</DialogDesc></DialogHeader>
          <Tabs defaultValue="excel" className="w-full pt-4" onValueChange={(value) => setActiveExportTab(value as 'excel' | 'barcodes')}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 p-1 border-b pb-4 mb-4">
                <div><Label htmlFor="exportStartDate">Start Date</Label><Input type="date" id="exportStartDate" value={exportFilters.startDate} onChange={(e) => handleExportFilterChange('startDate', e.target.value)} /></div>
                <div><Label htmlFor="exportEndDate">End Date</Label><Input type="date" id="exportEndDate" value={exportFilters.endDate} onChange={(e) => handleExportFilterChange('endDate', e.target.value)} /></div>
            ...
            <TabsContent value="excel" className="space-y-4">
                ...
                <ScrollArea className="max-h-60 w-full rounded-md border p-3"><div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2">{ALL_EXPORTABLE_FIELDS.map(field => (<div key={field.key} className="flex items-center space-x-2"><Checkbox id={`export-field-${field.key}`} checked={exportFilters.selectedFields.includes(field.key)} onCheckedChange={() => handleExportFieldToggle(field.key)}/><Label htmlFor={`export-field-${field.key}`} className="text-xs font-normal cursor-pointer">{field.label}</Label></div>))}</div></ScrollArea>
                 <div className="flex items-center justify-between gap-2 mt-2"><Button variant="outline" onClick={() => fetchExportData(true, false)} disabled={isExporting}>{isExporting && exportDataPreviewCount === null ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}Preview Data Count</Button>{exportDataPreviewCount !== null && <span className="text-sm text-muted-foreground">{exportDataPreviewCount} records match.</span>}</div>
            </TabsContent>
            <TabsContent value="barcodes" className="space-y-4">
                <p className="text-sm text-muted-foreground">This will export only the "Barcode Scanned" field for the selected criteria.</p>
                 <div className="flex items-center justify-between gap-2 mt-2">
                    <Button variant="outline" onClick={() => fetchExportData(true, true)} disabled={isExporting}>
                        {isExporting && exportDataPreviewCount === null ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}
                        Preview Barcode Count
                    </Button>
                    {exportDataPreviewCount !== null && <span className="text-sm text-muted-foreground">{exportDataPreviewCount} barcodes match.</span>}
                </div>
                {exportedBarcodesText && (
                    <div className="space-y-2">
                        <Label htmlFor="exportedBarcodesArea">Barcodes:</Label>
                        <div className="relative">
                        <ScrollArea className="w-full whitespace-pre-wrap rounded-md border max-h-25">
                            <Textarea 
                                id="exportedBarcodesArea" 
                                value={exportedBarcodesText} 
                                readOnly 
                                className="font-mono text-xs pr-10 max-h-25 resize-none"
                            />
                            ...
                            <Button 
                                ...
                                className="absolute top-2 right-6 h-6 w-6 z-10"
                                title="Copy barcodes"
                            >
          ...
          <DialogFooter className="mt-6">
            ...
            <Button onClick={async () => { // Made async
                if (activeExportTab === 'excel') {
                    await handleGenerateExcel(); // Wait for excel generation
                } else if (activeExportTab === 'barcodes') {
                    await handleExportBarcodesAsText(); // Wait for barcode text preparation
                }
            }} disabled={isExporting || exportDataPreviewCount === null || exportDataPreviewCount === 0}>
